...
The package intentionally applies stricter IDNA2008 and presentation-form policies than default UTS #46. The registrar excludes otherwise-valid vectors whose Unicode result contains an `NV8` or `XV8` code point and valid trailing-root inputs. It ignores `U1` because preprocessing uses `UseSTD3ASCIIRules=false`.
