'use strict';
// Register one release's applicable IdnaTestV2 vectors from version-local sources and generated metadata.
...
// Validate generated release metadata before it controls source and inventory assertions.
const validateMetadata = (metadata) => {
    ...
    for (const [name, value] of Object.entries(metadata.expectedSummary)) {
        assert.ok(Number.isSafeInteger(value) && value >= 0, `IdnaTestV2 metadata ${name} must be a nonnegative safe integer.`);
    }
};
...
module.exports = (subject, concernRoot, metadata) => {
    ...
    assert.equal(sourceVersion, metadata.sourceVersion, 'Unexpected IdnaTestV2 Unicode version.');
